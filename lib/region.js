...
var regionIdentifier = function (apikey) {
  ...
  this.get = function (country, zipCode, callback) {
    ...
    if (availableCountries.indexOf(countryIso.alpha3) > -1) {
      ...
      var countryConfig = require('../country/' + countryIso.alpha3);
      ...
      for (var i = 0; i < zipCodeToRegionMappings.length; i++) {
        ...
        if (
          m.list &&
          (
            m.list.includes(parseInt(zip))
            || (
              countryConfig.zipCodeFormat === 'numeric'
                ? m.list.some(
                  (l) => zip.startsWith(String(l).padStart(countryConfig.zipCodeLength, '0'))
                )
                : false
            )
          )
  ...
  this.detectWithGoogle = function (countryIso, zipCode, callback) {
    ...
    request.get(googleLink.replace('####', encodeURIComponent(param)), function (err, res, body) {
      ...
      function cleanAndCatchFaultyRegionName(region) {
        var region = region.short_name.toUpperCase().replace('.', '');

        if (/ENGLAND/i.test(region)) region = 'ENG';
        else if (/NORTHERN.*IRELAND/i.test(region)) region = 'NIR';
        else if (/SCOTLAND/i.test(region)) region = 'SCT';
        else if (/WALES/i.test(region)) region = 'WLS';

        return region;
      }

      if (!region) return callback('NOT FOUND', null, true);
      var result = countryIso.alpha2 + '-' + cleanAndCatchFaultyRegionName(region);
