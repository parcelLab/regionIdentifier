...
function getRegionByZipCodeOnStaticFiles(countryIso, zip) {
  ...
  for (const regionMapping of zipCodeToRegionMappings) {
    ...
    if (!regionMapping.high && zip === regionMapping.low) {
      return regionMapping.region;
    }

    if (parseInt(zip) >= regionMapping.low && parseInt(zip) <= regionMapping.high) {
      return regionMapping.region;
    }
  }
}
...
function regionIdentifier(apikey) {
  ...
  this.get = function (country, zipCode, callback) {
    if (!country || !zipCode) return callback(new Error('Country and zip code are required.'), null, false);

    // prepare
    const countryIso = resolveCountryToISO3(country);
    if (!countryIso) return callback(new Error('Could not resolve country information.'), null, false);
  ...
  this.detectWithGoogle = async function (countryIso, zipCode, callback) {
    try {
      const addressParam = `country:${countryIso.alpha2}|postal_code:${zipCode}`;
      const googleApiEndpoint = `https://maps.googleapis.com/maps/api/geocode/json?key=${this.apikey}&address=${addressParam}`;

      const { body } = await requestGet(googleApiEndpoint);
      ...
      const countryAndRegion = `${countryIso.alpha2}-${cleanAndCatchFaultyRegionName(region)}`;
      callback(null, countryAndRegion, true);
    } catch (error) {
